...
class MonoRefineLayer(BaseModule):
    def __init__(
        self,
        embed_dims=256,
        pc_range=None,
        ...
        super(MonoRefineLayer, self).__init__()
    ...
    def forward(
        ...
        # Handle refinement of other properties
        ...
        # Handle opacity
        if self.include_opa:
            delta_opa = output[..., 10:11]
            opa_final = anchor[..., 10:11] + delta_opa
            output = torch.cat([output[..., :10], opa_final, output[..., 11:]], dim=-1)
        ...
        if self.semantic_dim > 0:
            ...
            output = torch.cat([
                output[..., :self.semantic_start], 
                semantics_final, 
                output[..., (self.semantic_start + self.semantic_dim):]
            ], dim=-1)
        
        # Convert output to format needed by renderer
        # Position: convert to world coordinates
        ...
        # Scale: convert to actual range
        gs_scales = safe_sigmoid(output[..., 3:6])
        gs_scales = self.scale_range[0] + (self.scale_range[1] - self.scale_range[0]) * gs_scales

        # Opacity
        if self.include_opa:
            opas = safe_sigmoid(output[..., 10:11])
        else:
            opas = torch.ones_like(xyz[..., :1])

        # Spherical harmonics coefficients (empty in this example)
        ...
        if self.semantic_dim > 0:
            ...
            semantics = torch.zeros(*instance_feature.shape[:-1], 0,
                device=instance_feature.device, dtype=instance_feature.dtype)
        ...
        gaussian = GaussianPrediction(
            ...
            scales=gs_scales,
            rotations=rot_final,
            harmonics=shs.unflatten(-1, (3, -1)) if shs.shape[-1] > 0 else shs,
            opacities=opas,
            semantics=semantics
        )
